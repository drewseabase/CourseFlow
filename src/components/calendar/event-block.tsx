 ...
 * - Will support drag & drop (added in next milestone)
 */
...
import { Lock, Check, Minus, BookOpen, Briefcase, Moon, Calendar } from "lucide-react";
...
export function EventBlock({ event, style, onClick, onDragStart, onDragEnd }: EventBlockProps) {
  ...
  // Fixed events: Gray background, solid border
  ...
  return (
    <div
      className={`${baseClasses} ${isFixed ? fixedClasses : blockClasses} ${statusClasses} ${cursorClass}`}
      style={style}
      onClick={onClick}
...
function renderStatusIndicator(block: ScheduleBlock) {
  ...
  if (block.status === Status.SKIPPED) {
    return (
